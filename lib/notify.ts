...
function sourceReviewHtml(article: Article, reviewLabel: string, intro: string): string {
  const title = escapeHtml(
    article.status === 'rss_final_review'
      ? article.title ?? article.trigger_title ?? 'Untitled'
      : article.trigger_title ?? article.title ?? 'Untitled'
  );
  const preview = article.status === 'rss_final_review'
    ? article.summary ?? article.trigger_content
    : article.trigger_content;
  const snippet = preview ? escapeHtml(preview) : 'No feed description was provided.';
  const sourceUrl = escapeHtml(article.trigger_url);
  const reviewUrl = escapeHtml(`${appUrl()}/review/${article.id}`);

  return `<div style="margin:0;padding:32px 16px;background-color:#F4F0E8;font-family:Georgia,'Times New Roman',serif;color:#191713;">
  <div style="max-width:600px;margin:0 auto;background-color:#FDFBF6;border:1px solid rgba(25,23,19,0.25);border-radius:6px;overflow:hidden;">
    <div style="padding:20px 24px;border-bottom:3px solid #C8361E;">
      <div style="font-size:13px;letter-spacing:2px;text-transform:uppercase;color:#C8361E;font-weight:bold;">${escapeHtml(SITE_NAME)}</div>
      <div style="font-size:12px;color:#6B6459;margin-top:4px;">${escapeHtml(reviewLabel)}</div>
    </div>
    <div style="padding:24px;">
      <p style="margin:0 0 16px;font-size:15px;line-height:1.6;color:#403B33;">${escapeHtml(intro)}</p>
      <h1 style="margin:0 0 16px;font-size:24px;line-height:1.25;font-weight:bold;">${title}</h1>
      <p style="margin:0 0 20px;font-size:15px;line-height:1.6;color:#403B33;">${snippet}</p>
      <p style="margin:0 0 24px;font-family:Helvetica,Arial,sans-serif;font-size:13px;"><a href="${sourceUrl}">Open source article ↗</a></p>
      <a href="${reviewUrl}" style="display:inline-block;padding:12px 24px;background-color:#C8361E;color:#FDFBF6;text-decoration:none;font-family:Helvetica,Arial,sans-serif;font-size:14px;font-weight:bold;letter-spacing:0.5px;border-radius:4px;">Review in Desk</a>
    </div>
    <div style="padding:16px 24px;border-top:1px solid rgba(25,23,19,0.15);font-size:12px;color:#6B6459;">
      Article #${article.id} · from the ${escapeHtml(article.source_feed)} feed
    </div>
...
async function sendSourceReviewEmail(
  article: Article,
  subject: string,
  reviewLabel: string,
  intro: string,
  ...
  try {
    const res = await fetchFn('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        to: [to],
        subject,
        html: sourceReviewHtml(article, reviewLabel, intro),
      }),
    });
    if (!res.ok) {
      const body = await res.text();
...
export function sendRssFirstReviewEmail(article: Article, deps: NotifyDeps = {}): Promise<boolean> {
  return sendSourceReviewEmail(
    article,
    `New RSS item to review: ${article.trigger_title ?? `article #${article.id}`}`,
    'A new RSS item needs your first review',
    'Decide whether this source should enter the drafting pipeline. Nothing has been drafted or imaged yet.',
    deps
  );
}

export function sendFinalReviewEmail(article: Article, deps: NotifyDeps = {}): Promise<boolean> {
  return sendSourceReviewEmail(
    article,
    `Draft ready for final review: ${article.title ?? `article #${article.id}`}`,
    'A drafted article needs final approval',
    'The article draft is ready. Final approval is the only action that can authorize thumbnail generation and publication.',
    deps
  );
}
